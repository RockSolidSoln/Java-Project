...
public class WriteBack {
    ...
    public static void store(String dName, String dAids ,int dQty, String nName) throws IOException{ //takes the Donors as well as NGO's info
        ...
        try (FileWriter fw = new FileWriter(path,true);){       //appending at the end of each data
            ...
            System.out.println("\n|------------------------------------------------------------------------|");
            System.out.format("|%10s  |%11s |%10s |%10s |%10s |%10s| %10s|","Donor ","PhoneNumber","Aid","Quantity","Ngo","Manpower","Reserved"); //template for the table.
            System.out.println("\n|------------------------------------------------------------------------|");
            System.out.printf("| %10s |%11s |%10s |%10s |%10s |%10s|",dName, phono, dAids, dQty, nName, manpower,"Reserved");              //printinf each transaction
            System.out.println("\n|------------------------------------------------------------------------|");
        ...
        if(flag==0){    
            ...
            try( FileWriter fw = new FileWriter(path,true);){
                for(int i=0; i<dQty.size(); i++){
                    if(dQty.get(i)!=0){     //do not append sthe 0 quantity values in the file
