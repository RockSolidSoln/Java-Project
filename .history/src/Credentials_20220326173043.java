...
public class Credentials {
    ...
    public static ArrayList<String> readCredsFromFile(int flag) throws IOException { // flag is to check if user is Donor or Ngo
        ...
        if(flag==1){ //if Donor
            lines = Files.readAllLines(Paths.get("src/DocumentationFiles/DonorCredentials.csv")); //read from DonorCredentials file
        }
        else if(flag==2){
            lines = Files.readAllLines(Paths.get("src/DocumentationFiles/NgoCredentials.csv"));    //reads from NgoCredentials file
        } 
    ...
    public static void saveCredsToFile(Scanner sc, int flag, String name, String password, int num) throws IOException { 
        ...
        if(innerFlag==1){               //asks the user to try again as username already in the use
            ...
            if(flag==1){        //if the user is Donor
                try{
                    String filename= "src/DocumentationFiles/DonorCredentials.csv";    //stores the path of the file
